...
const stableRemoteStartupBufferMs = 60;
const constrainedRemoteStartupBufferMs = 120;
const weakRemoteStartupBufferMs = 180;
const maximumAdaptiveStartupBufferMs = 260;
...
export type RemoteOutputMode = "audible" | "held-silent" | "inactive";
...
export function shouldPrepareProgressiveRuntimeForSource(input: {
  ...
  return (
    ...
    (input.activePlaybackSource !== "full-local" || input.progressiveEngineType === "pcm")
  );
}
...
export function resolveRemoteOutputMode(input: {
  activePlaybackSource: ProgressivePlaybackSource;
  playbackStatus: RoomSnapshot["room"]["playback"]["status"] | null | undefined;
  hasRemoteSrcObject: boolean;
}) {
  if (!input.hasRemoteSrcObject) {
    return "inactive" satisfies RemoteOutputMode;
  }

  if (input.activePlaybackSource === "remote-stream") {
    return "audible" satisfies RemoteOutputMode;
  }
  ...
  if (
    ...
    return "held-silent" satisfies RemoteOutputMode;
  }

  return "inactive" satisfies RemoteOutputMode;
}

function applyRemoteOutputState(input: {
  remoteAudio: HTMLAudioElement | null;
  mode: RemoteOutputMode;
  userVolume: number;
  outputVolume?: number;
}) {
  ...
  if (input.mode === "held-silent") {
    input.remoteAudio.volume = 0;
    return;
  }

  input.remoteAudio.volume = input.outputVolume ?? input.userVolume;
}
...
export function useProgressiveRuntime({
  ...
  const remoteOutputMode = useMemo(
    () =>
    ...
    [activePlaybackSource, playback?.status, remoteAudioRef, playbackRevision]
  );
  ...
  useEffect(() => {
    ...
    setActivePlaybackSource(
      canUseFullLocalForPlaybackSession ? "full-local" : "progressive-local"
    );
    setProgressiveFallbackReason(null);
  }, [
    activePlaybackSource,
  ...
  const scheduleRemoteStartupGate = useCallback(() => {
    ...
    if (activePlaybackSource !== "remote-stream" || playback?.status !== "playing") {
      remoteStartupReadyAtRef.current = null;
      applyRemoteOutputState({
        remoteAudio,
        mode: resolveRemoteOutputMode({
          activePlaybackSource,
          playbackStatus: playback?.status,
    ...
    applyRemoteOutputState({
      ...
      mode: "audible",
      userVolume: volume,
      outputVolume: volume * gateState.volumeRamp
    });
  ...
  useEffect(() => {
    ...
    const shouldUsePcmFullLocal = shouldUsePcmEngineForFullLocal({
      activePlaybackSource,
      forceSourceOwnerLocalPlayback,
      sourceOwnerHasLocalTrack,
      hasFullLocalTrack: !!uploaded,
      progressiveEngineType: currentProgressiveEngineType
    });
    ...
    if (wantsFullLocalPlayback && uploaded) {
      ...
      if (shouldUsePcmFullLocal) {
        ...
        if (pcmEngine) {
          ...
          setMediaConnectionState(playback.status === "playing" ? "buffering" : "idle");
        }
        return;
      }
  ...
  useEffect(() => {
    ...
    const shouldUsePcmFullLocal = shouldUsePcmEngineForFullLocal({
      activePlaybackSource,
      forceSourceOwnerLocalPlayback,
      sourceOwnerHasLocalTrack: isCurrentSourceOwner && !!currentBufferedFullLocalTrack,
      hasFullLocalTrack: !!currentBufferedFullLocalTrack,
      progressiveEngineType: currentProgressiveEngineType
    });

    if (!playbackState?.currentTrackId || !audio || !currentProgressiveManifest || !shouldUsePcmFullLocal) {
      return;
    }

    let cancelled = false;

    const syncFullLocalPcm = async () => {
      const pcmEngine = progressivePcmEngineRef.current;
      if (cancelled || !pcmEngine) {
        setMediaConnectionState(playbackState.status === "playing" ? "buffering" : "idle");
        return;
      }
      ...
      try {
        const result = await pcmEngine.syncPlayback(
          expectedSeconds,
    ...
    recordPeerDiagnostic({
      ...
      update: (snapshot) => ({
        ...
        progressivePlaybackStatus: {
          ...
          pcmDecodedPacketCount: pcmEngineDiagnostics?.decodedPacketCount ?? null,
          pcmDecoderFlushCount: pcmEngineDiagnostics?.decoderFlushCount ?? null,
          pcmLastDecodedAtMs: pcmEngineDiagnostics?.lastDecodedAtMs ?? null,
          pcmLastDecodeError: pcmEngineDiagnostics?.lastDecodeError ?? null,
          pcmDecodedPeak: pcmEngineDiagnostics?.decodedPeak ?? null,
          pcmDecodedRms: pcmEngineDiagnostics?.decodedRms ?? null,
          pcmDecodedNonZeroSampleCount: pcmEngineDiagnostics?.decodedNonZeroSampleCount ?? null,
          pcmBufferedAheadMs: pcmEngineDiagnostics?.bufferedAheadMs ?? null,
          pcmPlayoutState: pcmEngineDiagnostics?.playoutState ?? null,
