    ...
    >
      <div className="relative min-h-0 flex-1 overflow-hidden" data-testid="room-lyrics-lines">
        ...
        {status === "loading" ? (
          ...
          <div className={`flex min-h-full flex-col justify-center text-left ${isFiveLineView ? "gap-0 py-1 sm:gap-0.5 sm:py-2" : "gap-1 py-2 sm:gap-2 sm:py-3"}`}>
            {lines.map((line, index) => {
              ...
              return (
                <p
                  ...
                  className={`flex ${isActive ? (isFiveLineView ? "min-h-[4rem] sm:min-h-[4.5rem]" : "min-h-[3.5rem] sm:min-h-[4rem]") : (isFiveLineView ? "min-h-[2.5rem] sm:min-h-[3rem]" : "min-h-[2.25rem] sm:min-h-[2.75rem]")} shrink-0 max-w-[30rem] items-center break-words leading-[1.35] transition-[color,opacity] duration-300 ${
                    isActive
