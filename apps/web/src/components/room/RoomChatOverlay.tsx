...
export function RoomChatPanel({ roomId, activeSession, isHost, socket, scrollEnabled = true, onActivateScroll }: RoomChatPanelProps) {
  ...
  return (
    ...
    >
      <div
        className={`hide-scrollbar min-h-0 flex-1 px-3 py-2.5 sm:px-5 sm:py-3 ${scrollEnabled ? "touch-pan-y overflow-y-auto overscroll-contain" : "overflow-hidden"}`}
        onScroll={(event) => {
      ...
      <form className="flex shrink-0 gap-2 border-t border-surface-border/60 p-2.5 sm:p-3 bg-background/80 backdrop-blur-md" onSubmit={handleSend}>
