...
export function RoomChatPanel({ roomId, activeSession, isHost, socket, scrollEnabled = true, onActivateScroll }: RoomChatPanelProps) {
  ...
  return (
    <section
      className="flex h-full min-h-[24rem] min-w-0 flex-1 flex-col bg-transparent lg:min-h-0"
    ...
    >
      <div
        className={`hide-scrollbar min-h-0 flex-1 px-3 py-3 sm:px-4 sm:py-3.5 ${scrollEnabled ? "touch-pan-y overflow-y-auto overscroll-contain" : "overflow-hidden"}`}
        onScroll={(event) => {
          if (event.currentTarget.scrollTop < 48) void loadOlder();
        }}
        ref={messagesRef}
      >
        {isLoading ? <p className="py-8 text-center text-xs text-foreground-muted">正在加载聊天记录...</p> : null}
        {isLoadingOlder ? <p className="pb-2 text-center text-xs text-foreground-muted">正在加载更早消息...</p> : null}
        {errorMessage ? <p className="pb-2 text-center text-xs text-danger" role="status">{errorMessage}</p> : null}
        {!isLoading && !errorMessage && !messages.length ? <p className="py-8 text-center text-xs text-foreground-muted">还没有消息，发送第一条弹幕吧！</p> : null}
        <div className="space-y-3">
          {messages.map((message) => {
            const isCurrentUser = message.senderId === activeSession?.userId;
            return (
              <article className={`group flex min-w-0 items-start gap-2.5 ${isCurrentUser ? "justify-end" : "justify-start"}`} key={message.id}>
                ...
                <div className={`min-w-0 max-w-[min(82%,32rem)] ${isCurrentUser ? "text-right" : "text-left"}`}>
                  <div className={`mb-1 flex min-w-0 items-baseline gap-1.5 px-0.5 ${isCurrentUser ? "justify-end" : "justify-start"}`}>
                    ...
                    <time className="shrink-0 text-[10px] sm:text-xs tabular-nums text-foreground-muted/60" dateTime={new Date(message.timestamp).toISOString()}>{formatChatTime(message.timestamp)}</time>
                  ...
                  <div className={`inline-block max-w-full rounded-[0.875rem] px-3 py-2 text-left shadow-xs ${isCurrentUser ? "bg-accent text-white" : "bg-surface-elevated border border-surface-border/60 text-foreground"}`}><p className="break-words text-xs sm:text-sm leading-5">{message.content}</p></div>
                </div>
      ...
      <form className="material-surface-header flex shrink-0 gap-2 border-t border-surface-border/60 p-2 sm:p-2.5" onSubmit={handleSend}>
        <label className="sr-only" htmlFor={`radio-chat-input-${roomId}`}>发送消息</label>
        <input
          className="min-w-0 flex-1 rounded-xl border border-surface-border/60 bg-surface-elevated/40 px-3 py-2 text-xs sm:text-sm text-foreground outline-none transition focus:border-accent focus:ring-2 focus:ring-accent/20 disabled:cursor-not-allowed disabled:opacity-60 placeholder:text-foreground-muted/50"
          ...
          value={inputValue}
        />
        <Button disabled={!inputValue.trim() || !socket || !activeSession} size="sm" type="submit" className="rounded-xl px-3.5 py-2 text-xs">发送</Button>
      </form>
    </section>
  );
}
...
function ChatAvatar({ name, currentUser = false }: { name: string; currentUser?: boolean }) {
  return <span aria-hidden="true" className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-full text-xs font-semibold border border-surface-border/60 ${currentUser ? "bg-accent/15 text-accent" : "bg-surface-elevated text-foreground-muted"}`}>{name.slice(0, 1).toUpperCase()}</span>;
}
