    ...
    <div className="mt-7 grid gap-7">
      <section>
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,17rem),1fr))] gap-3">
          {(showAllOwned ? ownedRooms : ownedRooms.slice(0, 6)).map((snapshot) => <OwnedRoomCard key={snapshot.room.id} snapshot={snapshot} />)}
          {ownedRooms.length === 0 ? <EmptyState text="还没有创建房间" /> : null}
        </div>
        {ownedRooms.length > 6 ? <button className="mt-4 text-sm font-medium text-accent transition hover:text-accent/80" onClick={() => setShowAllOwned((current) => !current)} type="button">{showAllOwned ? "收起" : `查看全部 ${ownedRooms.length} 个房间`}</button> : null}
      </section>

      <section>
        <h2 className="mb-3 text-base font-semibold text-foreground">最近参与</h2>
        <div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,17rem),1fr))] gap-3">
          {(showAllRecent ? recentRooms : recentRooms.slice(0, 6)).map((room) => <RecentRoomCard key={room.roomId} room={room} />)}
          {recentRooms.length === 0 ? <EmptyState text="还没有参与记录" /> : null}
        </div>
        {recentRooms.length > 6 ? <button className="mt-4 text-sm font-medium text-accent transition hover:text-accent/80" onClick={() => setShowAllRecent((current) => !current)} type="button">{showAllRecent ? "收起" : `查看全部 ${recentRooms.length} 条记录`}</button> : null}
      </section>
...
function RecentRoomCard({ room }: { room: RoomActivitySummary }) {
  return (
    <Link className="border border-surface-border bg-surface/45 p-4 transition hover:border-accent/60 hover:bg-surface-hover" href={`/room/${room.roomId}`}>
      <div className="flex items-center justify-between gap-3">
        <span className="min-w-0 truncate font-medium text-foreground">{room.roomName}</span>
      ...
      <div className="mt-3 flex items-center justify-between gap-3 text-xs text-foreground-muted">
        ...
        <span className="shrink-0">{formatDuration(room.durationMs)}</span>
      </div>
    </Link>
  );
}
