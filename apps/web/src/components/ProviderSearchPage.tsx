    ...
    <main className="relative min-h-screen overflow-hidden bg-black pb-24 text-foreground selection:bg-accent/30 selection:text-white md:pl-60">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-[1280px] flex-col px-4 pb-20 pt-24 sm:px-6 md:mx-0 md:max-w-[1400px] md:px-8 md:pt-28">
        ...
        {enabledProviders.length > 0 ? (
          <>
            <div className="mt-8 flex max-w-md gap-1 rounded-xl border border-surface-border bg-surface/40 p-1" role="tablist" aria-label="音乐平台">
    ...
    <section className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.7fr)]">
      <div className="overflow-hidden rounded-xl border border-surface-border">
        {results.length ? results.map((track) => (
          <article className="flex items-center justify-between gap-3 border-b border-surface-border bg-surface/25 px-4 py-3 last:border-b-0" key={`${track.provider}-${track.providerTrackId}`}>
            ...
            <div className="flex shrink-0 items-center gap-1">
              <Button disabled={pending !== null} onClick={() => void onLyrics(track)} size="sm" variant="ghost" type="button">
                {pending === `lyrics:${track.providerTrackId}` ? "加载中…" : "查看歌词"}
              </Button>
              {track.providerAlbumId ? <Button disabled={pending !== null} onClick={() => void onAlbum(track)} size="sm" variant="ghost" type="button">专辑</Button> : null}
            </div>
          </article>
