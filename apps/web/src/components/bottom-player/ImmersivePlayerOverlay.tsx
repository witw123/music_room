...
export function ImmersivePlayerOverlay({
  ...
  return (
    <div
      aria-hidden={!isOpen}
      className={`fixed inset-0 z-[50] h-[100dvh] max-h-[100dvh] w-full overflow-hidden bg-[#05080d] text-foreground transition-[opacity,transform,visibility] duration-500 ease-out motion-reduce:transition-none ${isOpen ? "visible translate-y-0 opacity-100" : "pointer-events-none invisible translate-y-3 opacity-0"}`}
      role="dialog"
      aria-modal="true"
      aria-label="沉浸式播放"
    >
      ...
      <main className="relative mx-auto grid h-[100dvh] min-h-0 max-h-[100dvh] w-full max-w-[1500px] grid-rows-[minmax(0,1fr)_minmax(0,1fr)] items-center gap-4 overflow-hidden px-4 pb-4 pt-20 sm:gap-6 sm:px-8 sm:pb-6 sm:pt-24 lg:grid-cols-[minmax(0,1.08fr)_minmax(22rem,0.92fr)] lg:grid-rows-1 lg:gap-12 lg:px-14 lg:pb-8 lg:pt-12 xl:gap-20">
...
function ImmersiveLyrics({
  ...
  return (
    <div className="mt-3 min-h-0 border-t border-white/[0.08] pt-4 sm:mt-5 sm:pt-5">
      <RoomLyricsPanel
        className="h-[min(24svh,11rem)] max-h-[11rem] min-h-[5rem] sm:h-[min(30svh,15rem)] sm:max-h-[15rem] sm:min-h-[7rem]"
        isPlaying={isPlaying}
        lyrics={plainLyric}
        positionMs={positionMs}
        status={lyricsStatus}
