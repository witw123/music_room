...
const tasteRules: TasteRule[] = [
  { dimension: "genre", label: "流行", pattern: /(?:\bpop\b|流行|主打)/iu },
  ...
  { dimension: "genre", label: "摇滚", pattern: /(?:\brock\b|朋克|金属|\bmetal\b|\bpunk\b|硬核)/iu },
  ...
  { dimension: "genre", label: "爵士", pattern: /(?:\bjazz\b|\bblues\b|爵士|蓝调|bossa\s*nova)/iu },
  { dimension: "genre", label: "古典", pattern: /(?:\bclassical\b|古典|交响|协奏|奏鸣|钢琴曲)/iu },
  { dimension: "genre", label: "独立音乐", pattern: /(?:\balternative\b|\bindie\b|独立音乐|独立流行)/iu },
  { dimension: "genre", label: "轻音乐", pattern: /(?:\bambient\b|\bchill(?:out)?\b|lo[\s-]?fi|轻音乐|纯音乐|氛围音乐|白噪音)/iu },
  ...
  { dimension: "genre", label: "City-Pop", pattern: /(?:city[\s-]?pop|城市流行|vaporwave|蒸汽波)/iu },
  { dimension: "genre", label: "舞曲", pattern: /(?:\bdance\b|\bdisco\b|舞曲|迪斯科)/iu },
  { dimension: "genre", label: "影视原声", pattern: /(?:\bost\b|original\s+soundtrack|\bsoundtrack\b|原声|主题曲|片尾曲|插曲)/iu },
  { dimension: "genre", label: "K-pop", pattern: /(?:k[\s-]?pop|韩国流行|韩流)/iu },
  { dimension: "genre", label: "J-pop", pattern: /(?:j[\s-]?pop|日本流行|日系摇滚)/iu },
  { dimension: "language", label: "粤语", pattern: /(?:cantonese|粤语|粤语歌|港乐)/iu },
  { dimension: "language", label: "英语", pattern: /(?:english|英文|英语|欧美)/iu },
  { dimension: "language", label: "日语", pattern: /(?:japanese|日语|日文)/iu },
  { dimension: "language", label: "韩语", pattern: /(?:korean|韩语|韩文)/iu },
  { dimension: "language", label: "华语", pattern: /(?:mandarin|国语|华语|中文|华语流行)/iu },
  { dimension: "region", label: "欧美", pattern: /(?:欧美|western|\beurope\b|\bamerica\b|uk\s+pop)/iu },
  ...
  { dimension: "scene", label: "通勤", pattern: /(?:\bcommute\b|\bdriv(?:e|ing)\b|公路|开车|通勤|旅途)/iu },
  { dimension: "scene", label: "学习", pattern: /(?:\bstudy\b|\bfocus\b|学习|专注|工作|自习|coding)/iu },
  { dimension: "scene", label: "运动", pattern: /(?:\bworkout\b|\bgym\b|\brunning?\b|跑步|运动|健身|燃脂)/iu },
  { dimension: "scene", label: "放松", pattern: /(?:\brelax\b|\bcalm\b|疗愈|治愈|冥想|\bmeditation\b|放空|周末)/iu },
  { dimension: "scene", label: "派对", pattern: /(?:\bparty\b|\bclub\b|派对|聚会|狂欢|嗨歌)/iu },
  { dimension: "scene", label: "现场氛围", pattern: /(?:\blive\b|现场|演唱会|音乐节)/iu }
];

const artistTasteMap: ReadonlyArray<{ pattern: RegExp; labels: ReadonlyArray<{ label: string; dimension: TasteEvidence["dimension"] }> }> = [
  { pattern: /(?:the\s+weeknd|frank\s+ocean|sza|bruno\s+mars|方大同)/iu, labels: [{ dimension: "genre", label: "R&B" }] },
  { pattern: /(?:linkin\s+park|coldplay|beyond|五月天|草东没有派对|万能青年旅店|oasis)/iu, labels: [{ dimension: "genre", label: "摇滚" }] },
  { pattern: /(?:avicii|calvin\s+harris|alan\s+walker|zedd|marshmello)/iu, labels: [{ dimension: "genre", label: "电子" }] },
  { pattern: /(?:宇多田ヒカル|米津玄師|yoasobi|aimer|radwimps|zutomayo)/iu, labels: [{ dimension: "genre", label: "J-pop" }, { dimension: "genre", label: "ACG" }, { dimension: "language", label: "日语" }] },
  { pattern: /(?:bts|blackpink|iu\b|newjeans|aespa|ive)/iu, labels: [{ dimension: "genre", label: "K-pop" }, { dimension: "language", label: "韩语" }] },
  { pattern: /(?:周杰伦|林俊杰|陶喆|王力宏|孙燕姿|陈奕迅)/iu, labels: [{ dimension: "genre", label: "流行" }, { dimension: "language", label: "华语" }] }
];
