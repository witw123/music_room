...
export class RealtimeService {
  ...
  private getEphemeralIceServers(input: {
    ...
    for (const protocol of protocols) {
      ...
      } else if (protocol === "tls" || protocol === "turns") {
  ...
  if (appDomain) {
    const derivedHost =
      process.env.TURN_PUBLIC_HOST_USE_APP_DOMAIN === "1" || appDomain.startsWith("turn.")
        ? appDomain
        : `turn.${resolveTurnBaseDomain(appDomain)}`;
...
function resolveTurnBaseDomain(appDomain: string) {
  const labels = appDomain.split(".").filter(Boolean);
