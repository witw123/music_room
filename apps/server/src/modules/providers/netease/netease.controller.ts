...
export class NeteaseController {
  ...
  async artwork(
    ...
    if (!rawUrl?.trim()) {
      throw new HttpException(
        createApiErrorResponse(errorCodes.validationFailed, "NetEase artwork URL is required."),
    ...
    const limiter = new Transform({
      transform(chunk: Buffer, _encoding, callback) {
        ...
        if (bytes > result.maxBytes) {
          callback(new Error("NetEase artwork exceeded the configured proxy size."));
          return;
        }
        callback(null, chunk);
      }
  ...
  private async streamAudio(
    ...
    response.setHeader(
      "Content-Disposition",
      `attachment; filename="netease-${trackId}.${result.fileType}"`
    );
