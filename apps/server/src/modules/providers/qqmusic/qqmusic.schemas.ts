...
export const qqMusicSearchSuggestQuerySchema = z.object({
  keywords: z.string().trim().min(1).max(100)
}).strict();
export const qqMusicTrackIdSchema = z.string().trim().min(1).max(128).regex(/^[a-zA-Z0-9_-]+$/);
export const qqMusicQualitySchema = z.enum(["standard", "high", "exhigh", "lossless", "hires"]);
...
export type QqMusicSearchSuggestQuery = z.infer<typeof qqMusicSearchSuggestQuerySchema>;
