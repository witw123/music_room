import { Controller, Delete, Get, Headers, HttpException, HttpStatus, Param, Post, Query, Req, Res, UnauthorizedException } from "@nestjs/common";
...
export class QqMusicController {
  constructor(private readonly service: QqMusicService, private readonly auth: AuthService) {}
  @Get("account") async getAccount(@Headers("x-session-token") token?: string) { return this.service.getAccountStatus(await this.user(token)); }
  @Post("account/qr/start") async startQr(@Headers("x-session-token") token?: string) { return this.service.startQrLogin(await this.user(token)); }
  @Get("account/qr/:attemptId/status") async qrStatus(@Param("attemptId") id: string, @Headers("x-session-token") token?: string) { return this.service.checkQrLogin(await this.user(token), id); }
  @Delete("account") async disconnect(@Headers("x-session-token") token?: string) { return this.service.disconnectAccount(await this.user(token)); }
  @Get("search") async search(@Query() query: Record<string, unknown>, @Headers("x-session-token") token?: string) { return this.service.searchTracks(await this.user(token), parseRequestBody(qqMusicSearchQuerySchema, query)); }
  @Get("tracks/:trackId") async track(@Param("trackId") id: string, @Headers("x-session-token") token?: string) { return this.service.getTrack(await this.user(token), qqMusicTrackIdSchema.parse(id)); }
  @Get("tracks/:trackId/audio") async audio(@Param("trackId") id: string, @Query("quality") quality: string | undefined, @Headers("range") range: string | undefined, @Headers("x-session-token") token: string | undefined, @Req() request: Request, @Res() response: Response) {
    const userId = await this.user(token); const parsedId = qqMusicTrackIdSchema.safeParse(id); if (!parsedId.success) throw new HttpException(createApiErrorResponse(errorCodes.validationFailed, "Invalid QQ Music track id."), HttpStatus.BAD_REQUEST);
    const parsedQuality = qqMusicQualitySchema.safeParse(quality ?? process.env.QQMUSIC_DEFAULT_QUALITY ?? "exhigh"); if (!parsedQuality.success) throw new HttpException(createApiErrorResponse(errorCodes.validationFailed, "Invalid QQ Music audio quality."), HttpStatus.BAD_REQUEST);
