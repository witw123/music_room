...
- 本地 IndexedDB 个人上传资源和分段 Opus 播放资产
- 单一 Segmented Opus/WebRTC RTP 媒体链路
- 官网展示入口 `/` 与客户端入口 `/app`

服务端只保存账号、房间、队列、播放状态和音频元数据，不保存音频文件。房间不提供缓存下载、音频资产互传或房间级缓存同步。IndexedDB 只用于恢复当前用户自己上传的歌曲及其本地生成的资产。
...
核心用户链路已经跑通，当前主要工作集中在多实例稳态、真实 WebRTC/Media 集成测试、长时间播放验证和统一观测能力。
