...
| `/app/profile` | 个人页：听歌画像 / 房间足迹 / 平台账号三个 Tab |
| `/app/settings` | 独立设置页：通用、播放策略、界面、播放、通知与推送、隐私与数据、账号 |
| `/rooms` | 房间与最近房间入口 |
| `/room/[roomId]` | 房间工作区 |
...
个人页头部为身份卡：头像、昵称、口味摘要（常听曲风自动生成）、累计收听统计，播放中显示“正在收听”信号；右上角提供设置入口与退出登录。

- **听歌画像**：口味标签为第一区块，点击标签直接开启对应偏好漫游电台；下方为最常播放歌曲、常听歌手与音源收听分布；屏蔽与负反馈管理收纳在页尾折叠入口
- **房间足迹**：房间与最近房间回顾
- **平台账号**：网易云 / QQ 音乐账号绑定与平台资料导入（设置页不再重复提供）
