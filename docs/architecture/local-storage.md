...
资产链接和转码状态；播放资产缺少数据库记录时直接从目录读取。首次选择目录时，已有的
完整浏览器音频缓存会迁入仓库并删除对应的 IndexedDB 大对象和分片。
...
首次选择目录时初始化或打开 `.music-room/repository.json`，自动迁移已有的完整
IndexedDB 音频缓存、资产和摘要，然后恢复该仓库的 catalog、播放资产和本地歌单。
迁移成功后删除浏览器中的完整音频 Blob 与资产分片；迁移失败的记录保留，等待下次重试。
...
`cachedTrackLibrary` 仅作为旧数据迁移入口，新的完整音频不再写入该表。
