...
- 注册/登录、房间创建/加入/恢复、成员权限、暂离房间恢复、共享队列、播放控制（player 权限）已打通
...
- 播放统一使用 SegmentedOpusEngine + shared AudioContext + WebRTC RTP 广播链路（曲目拥有者是唯一媒体源，provider 曲目支持监听端 offline fallback）
...
- 房间创建、加入、恢复与退出
- 首页展示入口与 `/app` 网页工作区分流
- 多人共享播放队列、播放控制（player 权限）与播放同步
- 本地音频导入、个人曲库恢复和歌单管理
- 可选的网易云和 QQ 音乐账号绑定、歌曲/歌单/专辑搜索和本地导入
- 收藏专辑、个人资料、设置、主题偏好和暂离房间恢复
- 通过 shared AudioContext + WebRTC RTP 广播 Track 发布分段 Opus 播放（曲目拥有者是唯一媒体源）
- 成员级连接、媒体、播放和音频诊断
...
- 播放依赖实时信令和曲目拥有者浏览器在线（拥有者是唯一媒体源）
- Redis 不可用时，依赖 Realtime 的播放控制请求会失败
- 本地上传曲目在拥有者离线时会暂停；provider 曲目支持监听端 offline fallback
- 生产部署仍限制为单个 server 实例，多实例房间权威尚未完成
