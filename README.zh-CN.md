...
房间不会在成员之间下载或交换音频资产。曲目拥有者从本地已经准备好的分段 Opus 播放资产发布 WebRTC 媒体流，监听成员只接收一条 RTP Opus 音频流。网易云、QQ 音乐和 B 站（哔哩哔哩）导入由服务端临时代理取流，随后仍进入客户端本地播放资产流程。
...
- **界面设计规范与系统通知**：
  - 全面落实克制收敛、高信息密度的防 AI 模板化（Anti-AI-Slop）设计规范，根除全局发光霓虹（`--accent-glow`），菜单与弹窗采用 100% 实体高对比表面杜绝透底干扰
  - 房间舞台背景重构为极简几何声波曲线与同心电波弧线，保留写实黑胶唱片质感
  - 根路由（`/`）工程级产品落地页：真实直观呈现四阶段声学数据流架构、三大房间模式对比与本地一键自建指南
  - 顶部通知系统：默认极简铃铛图标，新通知由右向左单条跑马灯平稳循环（CSS 容器查询单位 `100cqw` 驱动，支持悬停暂停），点击直达结构清爽的公告中心弹窗
- **歌词系统**：
  - 支持标准 LRC 滚动歌词与网易云 YRC 逐字精准歌词（通过 `lyric_new` 接口）
  - 支持双语翻译歌词与音译歌词并排呈现，行内语义高亮精准分离
...
- **搜索与发现**：
  ...
  - 支持本地曲库、网易云音乐、QQ 音乐与 B 站（分 P 播放与导入）跨平台歌曲、专辑与歌单搜索
- **播放架构**：
...
- **在线平台集成**：可选的网易云、QQ 音乐与 B 站账号绑定、歌曲/歌单/专辑搜索与本地导入曲库
- **个性化与体验**：收藏专辑、听歌画像个人页（口味标签漫游、最常播放、正在收听）、平台账号绑定、独立设置页、多套主题偏好、暂离房间自动恢复、客户端更新检测
